...
class Fritzbox:
    ...
    def __init__(self):
        ...
        # Connect to FritzBox
        self.connection = FritzConnection(address=self.ip, user=self.user, password=self.password)
    ...
    def get_call_history(self, limit=None):
        ...
        try:
            ...
            state = self.connection.call_action('X_AVM-DE_OnTel', 'GetCallList')
    ...
    def _extract_call_data(self, call_element):
        ...
        try:
            ...
            return {
                ...
                'type_name': self.CALL_TYPES.get(int(call_type), 'UNKNOWN') if call_type else 'UNKNOWN',
                'caller': caller or "",
                'called': called or "",
                ...
                for pattern in name_patterns:
                    try:
                        ...
                        if matches:
                            ...
                            name = name[:50].strip()
                            print(name)
                            if name and len(name) > 2:  # Ensure we have a meaningful name
                                return name
                    except re.error:
